...
impl ProjectActivityStats {
    pub async fn load_for_project(
        ...
        let completed_count = sqlx::query(
            "SELECT COUNT(*) as count FROM tasks WHERE project_id = ?1 AND completed_at_utc IS NOT NULL AND completed_at_utc >= ?2"
        )
        .bind(&project_id)
        ...
        let created_count = sqlx::query(
            "SELECT COUNT(*) as count FROM tasks WHERE project_id = ?1 AND created_at_utc >= ?2",
        )
        .bind(&project_id)
        .bind(since.to_rfc3339())
        .fetch_one(&mut **connection)
...
impl ChartManager<'_> {
    pub async fn load_project_activity_stats(
        ...
        // Get only projects that have either completed or created tasks in the last week
        let project_ids = sqlx::query(
            ...
            AND (
                (t.completed_at_utc IS NOT NULL AND t.completed_at_utc >= ?1)
                OR (t.created_at_utc >= ?1)
            )",
        )
        .bind(since.to_rfc3339())
        ...
        for row in project_ids {
            ...
            let project_stats =
                ProjectActivityStats::load_for_project(project_id, since, &mut connection).await?;
